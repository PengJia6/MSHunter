...
def benchmark(parase):
    ...
    for ms_id, info in df_microsatellites.iterrows():
        ...
        if curentMSNum % (batch * thread) == 0:
            logger.info("Build Benchmark: Total" + str(ms_number)+ "microsatelite, processing:" +
                        str(curentMSNum - batch * thread + 1) +
                        "-" + str(curentMSNum) + "(" + str(round(curentMSNum / ms_number * 100, 2)) + "%)")
    ...
    logger.info("Build Benchmark: Total", ms_number, "microsatelite, finish all!")
    result_list = bm_multi_run(thread=thread, datalist=tmp_window)
    # bm_write_vcf(outputfile, result_list)
    # bm_write_vcf_close(outputfile)
