...
def merge():
    ...
    for hap1_rec, hap2_rec in zip(hap1.fetch(), hap2.fetch()):
        if hap1_rec.info["check"] == "False" or hap2_rec.info["check"] == "False": continue
        ...
        if None == hap1_rec.alts or None == hap2_rec.alts:
            alts = ("Uno", "Uno")
        ...
        merged_rec.info["dis"] = "|".join([hap1_rec.info["dis"], hap2_rec.info["dis"]])
        merged_rec.info["var_type"] = "|".join([hap1_rec.info["var_type"], hap2_rec.info["var_type"]])
        merged_rec.info["var_type_list"] = ",".join([hap1_rec.info["var_type_list"], hap2_rec.info["var_type_list"]])
        merged_rec.info["var_detail"] = "!".join([str(hap1_rec.info["var_detail"]), str(hap2_rec.info["var_detail"])])
